...
namespace GameOfRevenge.Business.CacheData
{
    public static class CacheHeroDataManager
    {
        ...
        public static bool IsLoaded { get => isLoaded && HeroInfo != null; }
        ...
        public static IReadOnlyHeroDataRequirementRel GetFullHeroData(int heroId)
        {
            var item = HeroInfos.FirstOrDefault(x => x.Info.HeroId == heroId);
            if (item == null) throw new CacheDataNotExistExecption(HeroNotExist);
            else return item;
        }
